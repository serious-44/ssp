...
string CardDistribution::str() const
{
    ...
    for (size_t i=0; i<_handList.size(); i++)
    {
        ...
        char x[256];
        snprintf(x, 250, "%s=%.3f", hand.str().c_str(), weight(hand));
        ret += (i>0?",":"") + string(x);
...
void CardDistribution::fill (const CardSet& cs, int n)
{
    ...
    int vsize = binomial_coefficient(setsize, n);
    clear();
...
bool CardDistribution::parse(const std::string& input)
{
    ...
    while((pos = input.find(',', pos)) != std::string::npos) {
        ...
        hands.push_back(substring);
        prev_pos = ++pos;
    }
    hands.push_back(input.substr(prev_pos, pos-prev_pos));
