...
public:
    ...
    std::string index_str() const
    {
        ...
        for (size_t p=0; p<numParts(); p++)
        {
            ...
            {
                ret += (i ? " " : "") + std::to_string(getIndex(p,i));
            }
            ret += "}";
        }
        return ret;
     ...
     */
    size_t partSize (size_t p) const
    {
        return _parts[p];
    }
    ...
    uint64_t getMask (size_t partnum) const
    {
        return _masks[partnum];
    }
