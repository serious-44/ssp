...
double clampedChoose(int n, int m)
{
    ...
    //int s1 = boost::math::binomial_coefficient<double>(n,m);
    int s2 = binomial_coefficient(n,m);
    return (double) s2;
}
...
size_t CardSet::colex() const
{
    ...
    for (size_t i=0; i<cards.size(); i++)
    {
        ...
        if (code >= i+1) {
            ...
            //int s1 = boost::math::binomial_coefficient<double>(code, i+1);
            int s2 = binomial_coefficient(code, i+1);
            value += s2;
        }
#endif
    }
