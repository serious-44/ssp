...
int PokerEvaluation::reducedCode2to7() const
{
    ...
    switch (type())
    {
        ...
        case THREE_OF_A_KIND:
        {
            ...
            e = PokerEvaluation((TWO_PAIR<<VSHIFT) ^(ranks[2] << MAJOR_SHIFT) ^(ranks[1] << MINOR_SHIFT) ^(0x01<<ranks[0]));
            e.flip();
            return e.code();
        }
        ...
        case ONE_PAIR:
        {
            ...
            ranks[0] = lastbit(kickers);   kickers ^= 0x01<<ranks[0];
            ranks[1] = lastbit(kickers);   kickers ^= 0x01<<ranks[1];
            ranks[2] = lastbit(kickers);   kickers ^= 0x01<<ranks[2];
            ranks[3] = e.majorRank().code();
            sort(ranks.begin(), ranks.end());
            e = PokerEvaluation((ONE_PAIR<<VSHIFT) ^(ranks[2] << MAJOR_SHIFT) ^(0x01<<ranks[0]) ^(0x01<<ranks[1]));
            e.flip();
            return e.code();
        }
        ...
        case FULL_HOUSE:
        {
            ...
            sort(ranks.begin(), ranks.end());
            e = PokerEvaluation((FULL_HOUSE<<VSHIFT) ^(ranks[1]<<MAJOR_SHIFT) ^(ranks[0]<< MINOR_SHIFT));
            e.flip();
            return e.code();
        }
        ...
        case FOUR_OF_A_KIND:
        {
            ...
            ranks[0] = lastbit(kickers);
            ranks[1] = e.majorRank().code();
            e = PokerEvaluation((FOUR_OF_A_KIND<<VSHIFT) ^(ranks[1]<<MAJOR_SHIFT) ^(0x01<<ranks[0]));
            e.flip();
            return e.code();
        }
...
string PokerEvaluation::toStringCannon() const
{
    ...
    char x[256];
    snprintf(x, 250, "%s %-5s", hand.c_str(), ranks.c_str());
    ret = x;
...
void PokerEvaluation::generateLowballLookupA5()
{
    ...
    for (int i=0; i<=0x1FFF; i++)
    {
        ...
        char x[256];
        snprintf(x, 250, "    %6d, // %s\n", lowbits, toBitString(lowbits).c_str());
        cout << x;
